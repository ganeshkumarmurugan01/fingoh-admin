...
function PlatformEmailScreen() {
  ...
  const previewHtml = (bodyHtml) => {
    ...
    const logo     = config.logo_url
      ? `<img src="${config.logo_url}" style="max-height:44px;display:block;" alt="logo">`
      : `<span style="color:#fff;font-size:20px;font-weight:800;">${config.sender_name||"Fingoh"}</span>`;
...
<tr><td style="background:#F8FAFC;padding:12px 28px;border-top:1px solid #E2E8F0;text-align:center;">
  ...
  const currentTemplate = config.templates?.[currentKey] || "";
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:24,borderBottom:"2px solid #E2E8F0"}}>
        {EMAIL_TABS.map(t => (
          <button key={t.id} onClick={() => { setActiveEmailTab(t.id); setTestMsg(""); }}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          ...
          <div style={{gridColumn:"1/-1"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              {config.logo_url && (
                <div style={{background:config.primary_color||"#0D1B3E",borderRadius:8,padding:"10px 16px",display:"flex",alignItems:"center"}}>
                  <img src={config.logo_url} alt="logo" style={{maxHeight:40,maxWidth:160,display:"block"}}/>
                </div>
              )}
              <label style={{display:"inline-flex",alignItems:"center",gap:8,padding:"9px 16px",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:600,color:C.navy}}>
                📎 {config.logo_url ? "Change logo" : "Upload logo"}
                <input type="file" accept="image/*" onChange={handleLogoUpload} style={{display:"none"}}/>
              </label>
              {config.logo_url && (
                <button onClick={() => set("logo_url", null)}
                  style={{background:"none",border:"none",color:C.muted,fontSize:12,cursor:"pointer",fontFamily:F}}>
                  Remove
                </button>
              )}
            </div>
            <p style={{fontSize:11,color:C.muted,margin:"6px 0 0"}}>Or paste a URL below</p>
            <input value={config.logo_url||""} onChange={e=>set("logo_url",e.target.value)}
              placeholder="https://…/logo.png" style={{...iS,marginTop:6}}/>
          </div>

          {/* Primary colour */}
          <div>
            <label style={lS}>Primary Colour</label>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <input type="color" value={config.primary_color||"#0D1B3E"}
                onChange={e=>set("primary_color",e.target.value)}
                style={{width:44,height:38,padding:2,border:"1px solid #E2E8F0",borderRadius:8,cursor:"pointer",background:"none"}}/>
              <input value={config.primary_color||""} onChange={e=>set("primary_color",e.target.value)}
                placeholder="#0D1B3E" style={{...iS,flex:1}}/>
            </div>
          </div>

          {/* Sender name */}
          <div>
            <label style={lS}>Sender Name</label>
            <input value={config.sender_name||""} onChange={e=>set("sender_name",e.target.value)}
              placeholder="Fingoh" style={iS}/>
          </div>

          {/* Reply-to */}
          <div>
            <label style={lS}>Reply-To Email</label>
            <input value={config.reply_to||""} onChange={e=>set("reply_to",e.target.value)}
              placeholder="hello@fingoh.ai" style={iS}/>
          </div>

          {/* Banner */}
          <div>
            <label style={lS}>Banner URL (optional)</label>
            <input value={config.banner_url||""} onChange={e=>set("banner_url",e.target.value)}
              placeholder="https://…/banner.png" style={iS}/>
          </div>
          ...
          {[
            ...
            <div key={key}>
              ...
              <input value={config[key]||""} onChange={e=>set(key,e.target.value)} placeholder={ph} style={iS}/>
            </div>
          ))}
          ...
          {/* Live branding preview strip */}
          <div style={{gridColumn:"1/-1",borderRadius:10,overflow:"hidden",border:"1px solid #E2E8F0"}}>
            <div style={{background:config.primary_color||"#0D1B3E",padding:"14px 20px",display:"flex",alignItems:"center",gap:12}}>
              {config.logo_url
                ? <img src={config.logo_url} alt="logo" style={{maxHeight:36,display:"block"}}/>
                : <span style={{color:"#fff",fontSize:18,fontWeight:800}}>{config.sender_name||"Fingoh"}</span>
              }
            </div>
            <div style={{padding:"10px 20px",background:"#F8FAFC",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <p style={{margin:0,fontSize:13,fontWeight:700,color:config.primary_color||"#0D1B3E"}}>{config.signature_name||"The Fingoh Team"}</p>
                {config.signature_title && <p style={{margin:"2px 0",fontSize:12,color:C.muted}}>{config.signature_title}</p>}
                {config.signature_company && <p style={{margin:"2px 0",fontSize:12,color:C.muted}}>{config.signature_company}</p>}
              </div>
              <p style={{margin:0,fontSize:11,color:C.muted}}>{config.footer_text||"Sent via Fingoh"}</p>
            </div>
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          <div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
              <label style={{...lS,margin:0}}>
                {activeEmailTab==="verification" ? "Verification Email HTML" : "Welcome Email HTML"}
              </label>
              <button onClick={()=>sendTest(currentKey)} disabled={sendingTest}
                style={{padding:"5px 12px",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:6,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:F,color:C.navy}}>
                {sendingTest ? "Sending…" : "📨 Send test to my email"}
              </button>
            </div>
            {testMsg && (
              <p style={{fontSize:12,color:testMsg.startsWith("✓")?"#16A34A":C.red,margin:"0 0 8px",fontWeight:600}}>{testMsg}</p>
            )}
            <p style={{fontSize:11,color:C.muted,margin:"0 0 8px"}}>
              Variables:{" "}
              <code style={{background:"#F1F5F9",padding:"1px 5px",borderRadius:4}}>{"{{name}}"}</code>{" "}
              <code style={{background:"#F1F5F9",padding:"1px 5px",borderRadius:4}}>{"{{company}}"}</code>{" "}
              {activeEmailTab==="verification" && (
                <code style={{background:"#F1F5F9",padding:"1px 5px",borderRadius:4}}>{"{{verify_link}}"}</code>
              )}
            </p>
            <textarea
          ...
          <div>
            ...
            <iframe
              ...
              style={{width:"100%",height:580,border:"1px solid #E2E8F0",borderRadius:8,background:"#fff"}}
              title="email-preview"
            />
          </div>
        </div>
